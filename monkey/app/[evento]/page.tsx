...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!evento) return { title: 'Evento — Monkey Restobar' }

  const titulo = evento.nombre
  const descripcion =
    evento.descripcion ??
    `Solicita tu invitación gratuita para ${evento.nombre} en Monkey Restobar. Entrada gratuita con QR personal.`
  const imagen = evento.imagenUrl ? ogImageUrl(evento.imagenUrl) : null
  const url = `${BASE_URL}/${slug}`

  return {
    title: `${titulo} — Monkey Restobar`,
