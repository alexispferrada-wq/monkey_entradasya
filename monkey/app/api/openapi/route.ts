...
/**
 * GET /api/openapi
 * Returns the OpenAPI 3.0 specification for all Monkey Restobar API endpoints.
...
export async function GET() {
  const spec = {
    openapi: '3.0.3',
    info: {
      title: 'Monkey Restobar API',
      version: '1.0.0',
      description: 'API para gestión de eventos, invitaciones, socios y chatbot de Monkey Restobar.',
      contact: { email: 'admin@monkeyrestobar.cl' },
    },
    servers: [
      { url: process.env.NEXT_PUBLIC_BASE_URL || 'https://monkey.entradasya.cl', description: 'Producción' },
    ...
    paths: {
      ...
      '/api/socios': {
        post: {
          tags: ['public'],
          summary: 'Registrar nuevo socio del Club Monkey',
          requestBody: {
            required: true,
            content: {
              'application/json': {
      ...
      '/api/chat': {
        post: {
          tags: ['public'],
          summary: 'Chat con el asistente virtual de Monkey Restobar',
