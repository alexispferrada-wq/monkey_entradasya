...
const schema = z.object({
  ...
  lugar:             z.enum(['MONKEY LOUNGE', 'MONKEY GRILL', 'TERRAZA']),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const precioGrill = data.lugar === 'MONKEY GRILL' ? 10000 : 0
    const nombreEvento = `🎂 Cumpleaños de ${data.cumpleañeroNombre}`
    const descripcion = `Celebración privada · ${data.cumpleañeroNombre} cumple ${data.edad} años · ${data.cantidadInvitados} invitados esperados${precioGrill ? ' · Monkey Grill $10.000' : ''}`
