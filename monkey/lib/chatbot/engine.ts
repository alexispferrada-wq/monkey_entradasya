...
export function processMessage(
  ...
  if (matches(t, KW.whatsapp)) {
    return { reply: `¡Con gusto! Podés escribirnos directamente por WhatsApp y te atendemos al tiro 👉 [Contactar por WhatsApp](${waUrl}) 🐒` }
  }
  ...
  if (matches(t, KW.cumple)) {
    ...
    return { reply: `¡Celebremos juntos en Monkey! 🎂 Para coordinar tu evento especial, escríbenos por [WhatsApp](${waUrl}) y te armamos algo único.` }
  }
  ...
  if (matches(t, KW.ubicacion)) {
    const infoDocs = docsByCategory(docs, 'info')
    if (infoDocs.length > 0) {
      return { reply: `📍 Aquí nos encontramos:\n\n${formatDocs(infoDocs)}` }
    }
    return { reply: `📍 Estamos en **Av. Concha y Toro 1060, Local 3, Puente Alto**.\n\nSi tienes dudas de cómo llegar, escríbenos por [WhatsApp](${waUrl}) 🐒` }
  }
  ...
  return {
    reply: `Mmm, no estoy seguro de cómo ayudarte con eso 😅\n\nPuedo contarte sobre el **menú**, **horarios**, **eventos**, **reservas** o nuestra **ubicación**. O si preferís hablar directamente con alguien del equipo, por acá 👉 [WhatsApp](${waUrl}) 🐒`,
  }
}
